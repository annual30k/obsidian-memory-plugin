 ...
 *
 * Dataset: JSON Lines, one object per line: {"text": "...", "recall": true|false}.
...
export function loadDataset(filePath) {
  ...
  readFileSync(filePath, "utf8").split(/\r?\n/u).forEach((line, index) => {
    ...
    if (typeof item.text !== "string" || !item.text.trim() || typeof item.recall !== "boolean") {
      throw new TypeError(`Invalid dataset line ${index + 1}: need {"text": string, "recall": boolean}`);
    }
    rows.push({ text: item.text, recall: item.recall });
  });
  return rows;
}
...
export function computeMetrics(rows, thresholds = DEFAULT_THRESHOLDS) {
  ...
  const endToEnd = score(rows.map((row) => ({ predicted: row.recallRecommended, actual: row.recall })));
  ...
  return { total: rows.length, routes, endToEnd, gating, layaOnly: { n: layaRows.length, sweep, best } };
}
...
export async function runEval(argv = process.argv.slice(2), { stdout = process.stdout, routerFactory = null } = {}) {
  ...
  if (args.help) {
    stdout.write("Usage: node scripts/eval-laya.mjs [--data file.jsonl] [--endpoint http://127.0.0.1:PORT] [--service-file path] [--json]\n");
    return 0;
  }
  ...
  const config = parseMemoryJudgeConfig({ mode: "auto", timeout: 10000, coldStartTimeout: 60000, healthTimeout: 2000, ...args.config }); // recallThreshold: plugin default
  const router = routerFactory ? routerFactory(config) : new MemoryRouter(config, { useCache: false });

  const rows = [];
  try {
    for (const item of dataset) {
      const result = await router.evaluateRecall(item.text);
      rows.push({ recall: item.recall, route: result.trace?.route ?? "fallback", recallRecommended: Boolean(result.recallRecommended), score: typeof result.score === "number" ? result.score : null, reason: result.reason, memoryAction: result.memoryAction });
    }
  } finally {
    router.dispose?.();
  }
