  ...
  if (raw.endpoints && typeof raw.endpoints === "object" && !Array.isArray(raw.endpoints)) {
    for (const [ep, entry] of Object.entries(raw.endpoints)) {
      ...
      if (entry.health && typeof entry.health === "object" && !Array.isArray(entry.health)) {
        ...
        const isIdleUnloadValid = h.idleUnloadSeconds === undefined ||
          (Number.isInteger(h.idleUnloadSeconds) && h.idleUnloadSeconds >= 0 && h.idleUnloadSeconds <= 86400);
        const isLastInferenceValid = h.lastInferenceAt === undefined ||
          (Number.isFinite(h.lastInferenceAt) && h.lastInferenceAt >= 0);

        // ONLY retain health if all fields strictly match the schema; NEVER elevate illegal values!
        if (isStatusValid && isModelValid && isCheckedAtValid && isIdleUnloadValid && isLastInferenceValid) {
          cleanEntry.health = {
            status: h.status,
            modelStatus: h.modelStatus,
            checkedAt: h.checkedAt,
            ...(Number.isInteger(h.idleUnloadSeconds) ? { idleUnloadSeconds: h.idleUnloadSeconds } : {}),
            ...(Number.isFinite(h.lastInferenceAt) ? { lastInferenceAt: h.lastInferenceAt } : {})
          };
        }
      }
  ...
  if (entry.health && typeof entry.health === "object") {
    ...
    if (isStatusValid && isModelValid && isCheckedAtValid && (now - h.checkedAt <= healthTtlMs)) {
      health = {
        ...
        modelStatus: h.modelStatus,
        checkedAt: h.checkedAt,
        idleUnloadSeconds: Number.isInteger(h.idleUnloadSeconds) && h.idleUnloadSeconds >= 0 && h.idleUnloadSeconds <= 86400
          ? h.idleUnloadSeconds : null,
        lastInferenceAt: Number.isFinite(h.lastInferenceAt) && h.lastInferenceAt >= 0 ? h.lastInferenceAt : null
      };
    }
  }
...
export function writeStateCache(cachePath, payload = {}, options = {}) {
  ...
  if (endpoint && typeof endpoint === "string") {
    ...
    if (payload.health && typeof payload.health === "object") {
      ...
      if (isStatusValid && isModelValid && isIdleUnloadValid && isLastInferenceValid) {
        updatedEntry.health = {
          status: h.status,
          modelStatus: h.modelStatus,
