...
import { resolve, normalize, relative, isAbsolute, dirname, basename, win32 } from "node:path";
...
export function readTrustedServiceFile(filePath, options = {}) {
  ...
  const pathModule = options.pathModule ?? (platform === "win32" ? win32 : { resolve, normalize, relative, isAbsolute, dirname, basename });
  ...
  try {
    ...
    let endpoint;
    if (data.transport === "uds") {
      if (platform === "win32" || typeof data.socket_path !== "string" || data.socket_path.includes("\0")) return null;
      const socketPath = pathModule.resolve(data.socket_path);
      const expectedDir = pathModule.resolve(pathModule.dirname(resolved));
      if (pathModule.dirname(socketPath) !== expectedDir || pathModule.basename(socketPath) !== "service.sock") return null;
      const directoryStat = fs.lstatSync(expectedDir);
      if (directoryStat.isSymbolicLink?.() || !directoryStat.isDirectory?.() || (directoryStat.mode & 0o077) !== 0) return null;
      if (typeof getuid === "function" && directoryStat.uid !== getuid()) return null;
      const socketStat = fs.lstatSync(socketPath);
      if (!socketStat.isSocket?.() || (socketStat.mode & 0o777) !== 0o600) return null;
      if (typeof getuid === "function" && socketStat.uid !== getuid()) return null;
      endpoint = `uds:${socketPath}`;
    } else {
      endpoint = validateLoopbackEndpoint(data.endpoint);
    }
    let token = null;
