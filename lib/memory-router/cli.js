...
export async function runCli(args = process.argv.slice(2), { stdin = null, stdout = process.stdout } = {}) {
  ...
  try {
    ...
    } else if (result.captureRecommended) {
      const cat = result.captureCategory || "decision";
      const scope = result.scope === "global" ? "global" : "project";
      const targetDir = scope === "global" ? "10-Global/inbox/" : "project inbox/";
      guidanceAppend = `[Laya Memory Judge: high-value ${cat} detected (scope: ${scope}). Proactively stage candidate note to ${targetDir} with status: pending-ingest upon concluding task.]`;
    }
