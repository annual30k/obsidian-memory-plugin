  ...
  async evaluateRecall(text, projectContext = null) {
    ...
    if (fast.action !== "consult_laya") {
      ...
      return {
        ...
        scope: (fast.recallRecommended || captureRecommended) ? (fast.scope ?? "project") : null,
        reason: (isCaptureDisabled && fast.captureRecommended) ? "proactive_capture_disabled" : fast.reason
      };
    }
    ...
    if (!this.client && this.config.mode === "auto") {
      ...
      if (this.discoveryTimer) {
        return { recallRecommended: false, captureRecommended: false, captureCategory: null, reason: "no_trusted_service" };
      }
    ...
    if (!this.client) {
      return { recallRecommended: false, captureRecommended: false, captureCategory: null, reason: "unconfigured_endpoint" };
    }

    // 4. Circuit Breaker Check
    if (!this.circuitBreaker.canAttempt()) {
      return { recallRecommended: false, captureRecommended: false, captureCategory: null, reason: "circuit_breaker_open" };
    }
    ...
    if (!this.healthChecked) {
      ...
      if (
        ...
        return { recallRecommended: false, captureRecommended: false, captureCategory: null, reason: "health_handshake_failed" };
      }
    }
    ...
    try {
      ...
      if (
        ...
        return { recallRecommended: false, captureRecommended: false, captureCategory: null, reason: "laya_fallback" };
      }
      ...
      if (this.config.proactiveCapture) {
        ...
        // Proactive capture requires high category score (>= captureThreshold) and high category confidence (>= 0.70)
        // Only trigger capture if not already a recall query
