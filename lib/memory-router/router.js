...
import { evaluateFastPath, isGenericQuestion } from "./fast-path.js";
...
export class MemoryRouter {
  ...
  async _evaluateRecall(text, projectContext = null) {
    ...
    try {
      ...
      if (this.config.proactiveCapture && this.config.layaCapture) {
        ...
        // Laya's category answer always names *some* category, so on its own it fired "capture" for plain
        // coding questions (seen in Codex: "read a CSV and sum columns" -> pitfall). Only consider
        // capture when the memory-need score says the turn is not clearly self-contained.
        const selfContained = typeof result.requiresMemory === "number" && result.requiresMemory < this.config.skipThreshold;
        if (!generic && !recallRecommended && !selfContained && catConfidence >= 0.70) {
