...
export function requestServiceAutoRestart(serviceFile, options = {}) {
  ...
  try {
    if (fileSystem.existsSync(paths.lockFile)) {
      ...
      // v1 locks stored the long-lived host/requester PID in `pid`, which
      // could suppress recovery for as long as Codex/OpenClaw remained open.
      // v2 tracks the short-lived recovery CLI child instead.
      const isV2Lock = lock?.version === 2;
      const childPid = isV2Lock && Number.isInteger(lock.childPid) ? lock.childPid : null;
      const recoveryRunning = isV2Lock && !Number.isFinite(lock.finishedAt) && isPidRunning(childPid, processApi);
      const spawnPending = isV2Lock && childPid === null && !Number.isFinite(lock.finishedAt) && age < LOCK_STALE_MS;
      const legacyPending = !isV2Lock && !Number.isFinite(lock?.finishedAt) && age < LOCK_STALE_MS;
      const completedRecently = Number.isFinite(lock?.finishedAt) && now - lock.finishedAt < RETRY_DELAY_MS;
      if (recoveryRunning || spawnPending || legacyPending || completedRecently) {
    ...
    const lock = { version: 2, lockId, ownerPid: processApi.pid, childPid: null, startedAt: now, finishedAt: null };
    ...
    let child;
    try {
      child = spawn(processApi.execPath, [SERVICE_CLI, "start", "--recovery", "--home", layaHome], {
      ...
      updateLock({ finishedAt: Date.now() });
      return { scheduled: false, reason: "restart_unavailable" };
    }
    if (Number.isInteger(child?.pid) && child.pid > 0) updateLock({ childPid: child.pid });
    const markFinished = () => updateLock({ finishedAt: Date.now() });
