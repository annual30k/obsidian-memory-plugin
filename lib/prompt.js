...
export function buildGuidance(config, layaResult = null) {
  ...
  if (layaResult?.recallRecommended) {
    ...
    lines.push(`[Laya Memory Judge: recall recommended (scope: ${safeScope}). Search Vault before proceeding.]`);
  } else if (layaResult?.captureRecommended) {
    const cat = layaResult.captureCategory || "decision";
    const safeScope = layaResult.scope === "global" ? "global" : "project";
    const targetDir = safeScope === "global" ? "10-Global/inbox/" : "project inbox/";
    lines.push(`[Laya Memory Judge: high-value ${cat} detected (scope: ${safeScope}). Proactively stage candidate note to ${targetDir} with status: pending-ingest upon concluding task.]`);
  }
